      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="font-['Press_Start_2P'] text-xl md:text-3xl text-white mb-3">
            <span className="text-neon-yellow">*</span> EXPERIENCE
          </h2>
          <div className="w-24 h-1 bg-linear-to-r from-neon-yellow to-neon-cyan box-glow-cyan"></div>
        </motion.div>

        <div className="relative border-l-2 border-white/20 ml-3 md:ml-6 space-y-10">
          {experiences.map((exp, index) => (
            ...
            >
              {/* Timeline Dot */}
              <div
                className="absolute w-5 h-5 rounded-full -left-2.5 top-1 flex items-center justify-center bg-black border-2 z-10"
                style={{
              ...
              <div
                className="retro-pixel-border p-5 sm:p-6 hover:bg-white/5 transition-colors group relative overflow-hidden"
                data-robot-target={index === 0 ? "experience" : undefined}
              >
                <div
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-4 gap-4">
                  <div>
                    <h3
                      className="text-lg md:text-xl font-bold font-mono group-hover:text-glow-cyan transition-colors drop-shadow-[0_0_8px_currentColor]"
                      style={{ color: exp.color }}
                    >
                      {exp.role}
                    </h3>
                    <div className="flex items-center gap-2 text-gray-300 mt-2 font-mono text-xs">
                ...
                <ul className="space-y-2 mt-5">
                  {exp.description.map((item, i) => (
                    <li
                      key={i}
                      className="flex gap-3 text-gray-300 font-mono text-xs leading-loose"
                    >
                      <span
